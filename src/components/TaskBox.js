...
import DoneSVG from '../assets/images/DoneSVG.svg'   
import EditSVG from '../assets/images/EditSVG.svg'  
import DeleteSVG from '../assets/images/DeleteSVG.svg'  
import { useTranslation } from "react-i18next";
 
function TaskBox({id, title, description, completed, completed_at, createed_at, onClickTaskEditMode,  onClickTaskDelete,  onClickTaskDone, ...props } ) {
    const { t  } = useTranslation();   

    return (
   
        <div className='flex flex-col w-full h-auto lg:max-w-[60vw] border-blue-400 border-2 my-2  rounded-md'>
            <div className='flex flex-col w-full h-22 p-2    bg-blue-400 justify-between items-center'>
            ...
            <div className="flex flex-row items-center py-5 justify-around w-full h-12 border-t-2 border-blue-400 bg-blue-200 "> 
                <ButtonM src={DoneSVG}
                    altImg={t("list_task_box.task_box.done_button.alt")}
                    onClick={onClickTaskDone}  
                    className=" bg-green-500 rounded-full w-8 h-8 flex justify-center items-center hover:border-green-600 hover:bg-green-700 border-2 border-green-800 duration-500 "
                >

                </ButtonM>
                <ButtonM src={EditSVG}
                    altImg={t("list_task_box.task_box.edit_button.alt")}
                    onClick={onClickTaskEditMode} 
                    className=" bg-yellow-500 rounded-full w-8 h-8 flex justify-center items-center hover:border-yellow-600 hover:bg-yellow-700 border-2 border-yellow-800 duration-500 "
